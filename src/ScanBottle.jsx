...
import { clean, findBestMatch } from './helpers.js';
  ...
  async function recogniseLabel(file) {
    ...
    reader.onloadend = async () => {
      try {
        ...
        if (!response.ok) throw new Error(extracted.error || 'Recognition failed');
        ...
        if (match) {
          ...
          const wineToOpen = match.photoUrl ? match : await savePhotoForWine(match, file);
  ...
  return (
    <main>
      ...
      <section className="detail">
        ...
        <button onClick={() => onCreateWine({ producer: newProducer, wineName: newWineName, vintage: newVintage, photoFile })}><Plus /> Create new wine</button>
      </section>
    </main>
  );
}
