...
function wineRows(regionWines) {
  return regionWines.map(wine => {
    ...
    const details = appellation && !name.toLowerCase().includes(appellation.toLowerCase())
      ? [{ text: name, style: 'wineName' }, { text: appellation, style: 'appellation' }]
      : [{ text: name, style: 'wineName' }];

    return {
      margin: [22, 4, 0, 7],
      unbreakable: true,
      columns: [
        { width: '*', stack: [{ text: producer, style: 'producer' }, ...details] },
...
export function downloadWineListPdf(wines) {
  ...
  const docDefinition = {
    ...
    styles: {
      coverKicker: { fontSize: 10, color: '#5a1d27', bold: true, characterSpacing: 2 },
      ...
      country: { fontSize: 9, bold: true, color: '#17110f', characterSpacing: 1.2 },
